...
*/
/*
 * Phoenix-RTOS shared-system backend (fork of code/unix/unix_shared.c).
 *
 * Phoenix has no dlopen/dlsym (no <dlfcn.h>). In this port the game/cgame/ui
 * modules run as interpreted QVM bytecode (NO_VM_COMPILED) and the renderer
 * is statically linked (USE_RENDERER_DLOPEN=0), so the engine's only two
 * dynamic-load seams are never exercised:
 *   - vm.c's VMI_NATIVE path (Sys_LoadDll) is never taken: vm_* default to
 *     VMI_COMPILED and NO_VM_COMPILED forces VMI_BYTECODE.
 *   - the renderer's GetRefAPI is resolved at link time, not via
 *     Sys_LoadLibrary/Sys_LoadFunction.
 * The four Sys_Load* entry points below are therefore reduced to safe stubs
 * (they log and fail if ever called). Everything else — timing, file listing,
 * mkdir, FOpen, home/pwd paths, affinity — is kept verbatim. The unused
 * <pwd.h> (no getpwuid call) is dropped; engine-tree includes are rooted at
 * code/ (the port build passes -I<workdir>/code).
 */
#define _GNU_SOURCE
#include <sched.h>
#include <sys/types.h>
...
{
	/* Phoenix single-ELF port: no dlopen. With interpreted QVM
	 * (NO_VM_COMPILED) and a statically-linked renderer
	 * (USE_RENDERER_DLOPEN=0) nothing reaches this seam; fail loudly if
	 * some future native-VM/renderer-dlopen path ever tries to. */
	dll_err_count++;
	Com_Printf( "Sys_LoadLibrary(%s): dynamic loading is not supported on Phoenix-RTOS\n", name );
	return NULL;
}
...
{
	/* Nothing is ever loaded (see Sys_LoadLibrary). */
	(void)handle;
}
...
{
	/* No dynamic symbol table on Phoenix; see Sys_LoadLibrary. */
	(void)handle;
	(void)name;
	dll_err_count++;
	return NULL;

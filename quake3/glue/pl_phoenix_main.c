...
*/
/*
 * Phoenix-RTOS system/main backend (fork of code/unix/unix_main.c).
 *
 * Kept almost verbatim. Only two things differ from the Unix original:
 *   - The unused legacy SysV-IPC / process headers (<sys/ipc.h>, <sys/shm.h>,
 *     <sys/wait.h>) and <dlfcn.h> are dropped: unix_main.c never referenced
 *     shmget/fork/wait, and the dlopen seam lives in pl_phoenix_sys.c (the
 *     fork of unix_shared.c), not here. Phoenix has none of these headers.
 *   - The engine-tree includes are rooted at code/ (the port build passes
 *     -I<workdir>/code) since this fork lives outside code/unix/.
 * The tty/console path uses <termios.h>, which Phoenix provides.
 */
#include <unistd.h>
#include <signal.h>
#include <stdlib.h>
